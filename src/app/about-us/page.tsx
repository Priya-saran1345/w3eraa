...
basic_details && about&&
        ...
        <div>
            ...
            <div className='full'>
                ...
                <div className=' w-full bg-[url("/images/aboutwave.png")] flex items-end min-h-[345px]'>
                    <div className='w-full xl:w-[75%] mx-auto px-4 mt-8 sm:mt-0' >
                        <p className=' text-[32px] lg:text-[38px] font-bold text-center'>{about?.who_youare[0].title}</p>
                        <p className=' text-[18px] lg:text-[20px] px-10 text-center'  dangerouslySetInnerHTML={{ __html: about?.who_youare[0].description }} ></p>
                    </div>
                </div>
                                ...
                                return (
                                    <div className='bg-white rounded-md p-8 mt-6 group  md:w-[48%]' key={index}>
                                        ...
                                        <div className='flex flex-col gap-3 mt-4'>
                                            ...
                                            <p className=' text-[23px] lg:text-[26px] group-hover:text-pink font-medium'>{elem.title}</p>
                                            <p className=' text-[20px] lg:text-[22px] font-medium'>{elem.subtitle}</p>
                ...
                <div className='w-full  py-8 lg:py-16 bg-grey'>
                    <div className='md:w-[55%] mx-auto'>
                        ...
                        <p className=' text-[32px] lg:text-[48px] font-bold pb-4 text-center'> {about?.about_client[0]?.title}
                        </p>
                        <p className='text-[20px] fomt-medium text-center'>{about?.about_client[0]?.description}</p>
                    </div>
                                        ...
                                        <div className=' w-full max-w-[940px] h-[300px] flex  justify-between rounded-sm  bg-white'>
                                            ...
                                            <div className='p-6 w-[80%] flex flex-col overflow-y-scroll  justify-center'>
                                                <div className='border-l-2 mb-6 border-slate-800 pl-2'>
                                                    <p className='text-[24px] font-semibold mb-3'>{value?.title}</p>
                ...
                <div className=' pt-12 lg:pt-16 bg-white md:flex-row flex-col flex justify-between  items-start px-4 mx-auto xl:w-[75%]'>
                    <div className='md:w-[342px] text-center md:text-left mb-16 md:mb-0'>
                        <p className=' text-[32px] lg:text-[38px] font-semibold'>{about?.milestone[0]?.title}</p>
                ...
                <FooterBanner content={about?.ourapproach[0].title} slug={''}
                    btncontent={'Learn How We Work '}
                    description={about?.ourapproach[0].description} image={about?.ourapproach[0].image} />

                <div className='w-full bg-grey'>
                    <div className='w-full xl:w-[75%] mx-auto pb-16 px-4'>
                        <div className='md:w-[60%] mx-auto mb-8 py-4 text-center flex flex-col gap-3'>
                            <p className=' text-[32px] lg:text-[38px] font-semibold'>{about?.ourvision[0].title}</p>
                            <p className=' text-[18px] lg:text-[20px] font-medium'>{about?.ourvision[0].description} </p>
                        </div>
                        {
                            about?.ourvision_point.map((elem: any, index:any) => (
                                <div key={elem.id} className='bg-white mb-5 flex gap-10 group items-start py-4 px-4 md:px-16 rounded-lg'>
                                    ...
                                    <div>
                                        <p className='text-[26px] font-semibold group-hover:text-pink'>{elem.title}</p>
            ...
            <FooterBanner content={basic_details?.footer_card[0]?.title||''}
              description={basic_details?.footer_card[0]?.description}
              btncontent={'Talk to Our Expert '} slug={''} image={basic_details?.footer_card[0]?.image||'/images/girlFooter.png'} />
            <Footer />
        </div>
        }
        </div>
